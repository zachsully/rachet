...
(define flatten^ 
        (lambda (e need-var)
            (match e
                ...
                [`(if ,cnd ,thn ,els) (let-values ([(cnd_ex cnd_stmts) (flatten^ cnd need-var)]
                                                  [(thn_ex thn_stmts) (flatten^ thn need-var)]
                                                  [(els_ex els_stmts) (flatten^ els need-var)])
                                                 (let ([tmp (gensym "if")])
                                                   (values tmp 
                                                           (append cnd_stmts `((if (eq? #t ,cnd_ex) 
                ...
                [`(eq? ,e1 ,e2) (if need-var
                                  (let ([tmp (gensym "eq")])
                                    (let-values ([(e1_ex e1_stmts) (flatten^ e1 need-var)]
                                                 [(e2_ex e2_stmts) (flatten^ e2 need-var)])
                                                (values tmp (append e1_stmts e2_stmts `((assign ,tmp (eq? ,e1_ex ,e2_ex)))))))
                                    (let-values ([(e1_ex e1_stmts) (flatten^ e1 need-var)]
                                                 [(e2_ex e2_stmts) (flatten^ e2 need-var)])
                ...
                [`(not ,es) (if need-var
                              (let ([tmp (gensym "not")])
                                (let-values ([(es_ex es_stmts) (flatten^ es need-var)])
                                            (values tmp (append es_stmts `((assign ,tmp (not ,es_ex)))))))
                              (let-values ([(es_ex es_stmts) (flatten^ es need-var)])
