...
(define select-instructions^
  (lambda (e)
       (match e
         ...
         [`(assign ,v (- ,e1)) (let ([v^ (select-instructions^ v)]
                                     [e1^ (select-instructions^ e1)])
                                `((movq ,e1^ ,v^)
                                  (negq ,v^)))]
         [`(assign ,v (+ ,e1 ,e2)) (let ([v^ (select-instructions^ v)]
                                         [e1^ (select-instructions^ e1)]
         ...
         [`(assign ,v (not ,es)) `((movq ,(select-instructions^ es) ,(select-instructions^ v))
                                   (xorq (int 1) ,(select-instructions^ v)))]
         [`(if (eq? #t ,cnd) ,thn ,els) `((if (eq? (int 1) ,(select-instructions^ cnd)) 
                                 ,(select-instructions^ thn) 
                                 ,(select-instructions^ els)))]
         [`(assign ,v ,x) (let ([v^ (select-instructions^ v)]
...
(define select-instructions
  (lambda (e)
    (match e
      ...
      ['() '()]
      [else (append (select-instructions^ (car e)) (select-instructions (cdr e)))])))
